...
from data_preparation import sr_dataset

dataloader = torch.utils.data.DataLoader(
    sr_dataset, batch_size=1, num_workers=2, shuffle=True
)
data, corr = next(iter(dataloader))
...
class BiFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        P6_td = self.p6_td_conv_2(
            (self.p6_td_w1 * P6_td_inp + self.p6_td_w2 * self.p7_upsample(P7_td))
        ...
        P3_out = self.p3_out_conv_2(
            (self.p3_out_w1 * P3_td + self.p3_out_w2 * P4_td)
...
class ResNetBiFPN(nn.Module):
    def __init__(self):
        ...
        self.bifpn = BiFPN([64, 64, 128, 256, 512])

    def forward(self, data):
        x1, x2, x3, x4, x5 = self.resnet_encoder.get_features(data)
        features = self.bifpn([x1, x2, x3, x4, x5])
        return features


res = ResNetBiFPN()
img = next(iter(dataloader))[0]
f1, f2, f3, f4, f5 = res(img)

up_factor = 4
pixel_shuffle = nn.PixelShuffle(up_factor)
print(f1.shape)
shuffled = pixel_shuffle(f1)
